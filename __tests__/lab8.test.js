describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const card_count = await page.$('#cart-count')
    const text = await card_count.getProperty('innerText')
    const json = await text.jsonValue()
    expect(json).toBe('20');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let json = await text.jsonValue()
      expect(json).toBe('Remove from Cart')
    }
    const card_count = await page.$('#cart-count')
    let text = await card_count.getProperty('innerText')
    let json = await text.jsonValue()
    expect(json).toBe('20');
  }, 10000);
